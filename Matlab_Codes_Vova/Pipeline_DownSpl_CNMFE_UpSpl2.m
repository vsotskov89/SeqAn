...
Files = struct;
Files.folder = 'G:\Souris154148\22-06-30-DeepCadFirst\130014_sCMOS_154148-awake\';
Files.subfolderResults = [Files.folder,'4-Results\']; % generated files are saved in this subfolder;
Files.vessels = [ Files.subfolderResults, 'downsampled_Channel1Movie_Crop_Avg.tif' ]; % vessel cropped image that will be saved in the results structure;
% !!!! changer les deux prochaines lignes
%Files.subfolderData = [Files.folder,'1-Preprocess\']; % subfolder containing experimental data files;
%Files.basename = 'Substack_preprocess-'; 
if usingDeepCAD == true
    Files.subfolderData = [Files.folder,'5-DeepCAD\']; % subfolder containing experimental data files;
else
    Files.subfolderData = [Files.folder,'2-Cropped\']; % subfolder containing experimental data files; 
end
Files.basename = 'Substack_cropped-';
Files.matlab_file = 'C:\Users\ventalon\Documents\MATLAB\Preprocess\Pipeline_DownSpl_CNMFE_UpSpl2.m'; % to keep track of parameters
%Files.trackPosition = 'trackPosition.mat'; % file holding variable trackPosition as 2 columns matrice: time (s) and normalized [0-1] position
Files.results = 'Results.mat'; % output files
