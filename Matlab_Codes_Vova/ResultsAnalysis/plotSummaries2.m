    ...
    normFact = max(max(Neurons.SummedPlaceActivity(neuron).Dir1),max(Neurons.SummedPlaceActivity(neuron).Dir2))/Experiment.Dir1.NbPassage*1.05;
        ...
        subplot(2,10,2:5);
            ...
            if Neurons.isPC(neuron).Dir1
               ...
               ... %'Peak at ' num2str(100*round(Neurons.Peak(neuron).Dir1,3)) '% of theor. max ; Stability : ' num2str(100*round(Neurons.Stab(neuron).Dir1,3)) '%']) 
                 'Act in PF = ' num2str(Neurons.ActvityInPFvsOutPF(neuron).Dir1, '%.1f') ' x act out PF; Reliability: ' num2str(100*round(Neurons.Stab(neuron).Dir1,3)) '%'])
        ...
        subplot(2,10,7:10);
            ...
            if Neurons.isPC(neuron).Dir2
                ...
                'Act in PF = ' num2str(Neurons.ActvityInPFvsOutPF(neuron).Dir2, '%.1f') ' x act out PF; Reliability: ' num2str(100*round(Neurons.Stab(neuron).Dir2,3)) '%'])
             subplot(2,10,6)
