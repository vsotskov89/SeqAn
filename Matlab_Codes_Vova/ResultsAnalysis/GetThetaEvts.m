function ThetaEvts = GetThetaEvts(FiltActTheta,FiltActFreqNorm,Norm,varargin)
...
if ~isdmatrix(FiltActTheta) | size(FiltActTheta,2) ~= 2,
...
for i = 1:2:length(varargin),
	...
	switch(lower(varargin{i})),
		...
		case 'noise',
			noise = varargin{i+1};
			if ~isdmatrix(noise) | size(noise,1) ~= size(FiltActTheta,1) | size(noise,2) ~= 2,
...
% Square and normalize Theta signal
time = FiltActTheta(:,1);
signal = FiltActTheta(:,2);
squaredSignal = signal.^2;
window = ones(windowLength,1)/windowLength;
keep = [];

if Norm
    signalN = FiltActFreqNorm(:,2);
    squaredSignalN = signalN.^2;
    normalizedSquaredSignal = Filter0(window,sum(squaredSignal,2));
    normalizedSquaredSignalN = Filter0(window,sum(squaredSignalN,2));
    [normalizedSquaredSignal,sd]=unity(normalizedSquaredSignal./normalizedSquaredSignalN,sd,keep);
else
...
ThetaEvts = [time(thirdPass(:,1)) time(peakPosition) time(thirdPass(:,2)) peakNormalizedPower];
duration = ThetaEvts(:,3)-ThetaEvts(:,1);
ThetaEvts(duration<minRippleDuration/1000,:) = [];
% disp(['After min duration test: ' num2str(size(ripples,1)) ' events.']);

% Discard ripples that are way too long
duration = ThetaEvts(:,3)-ThetaEvts(:,1);
ThetaEvts(duration>maxRippleDuration/1000,:) = [];
...
if ~isempty(noise),
	...
	excluded = logical(zeros(size(ThetaEvts,1),1));
	% Exclude ripples when concomittent noise crosses high detection threshold
	previous = 1;
	for i = 1:size(ThetaEvts,1),
		j = FindInInterval(noise,[ThetaEvts(i,1),ThetaEvts(i,3)],previous);
		previous = j(2);
		if any(normalizedSquaredNoise(j(1):j(2))>highThresholdFactor),
			excluded(i) = 1;
		end
	end
	bad = ThetaEvts(excluded,:);
	ThetaEvts = ThetaEvts(~excluded,:);
...
%  		MultiPlotXY(time,signal,time,squaredSignal,time,normalizedSquaredSignal);
	...
	for i = 1:nPlots,
		...
		hold on;
  		yLim = ylim;
		for j=1:size(ThetaEvts,1),
			plot([ThetaEvts(j,1) ThetaEvts(j,1)],yLim,'g-');
			plot([ThetaEvts(j,2) ThetaEvts(j,2)],yLim,'k-');
			plot([ThetaEvts(j,3) ThetaEvts(j,3)],yLim,'r-');
			if i == 3,
				plot([ThetaEvts(j,1) ThetaEvts(j,3)],[ThetaEvts(j,4) ThetaEvts(j,4)],'k-');
			end
		end
		for j=1:size(bad,1),
