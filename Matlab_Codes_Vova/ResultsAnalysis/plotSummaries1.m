function plotSummaries(Experiment,Neurons,Params,results)

for neuron =1:Experiment.nNeurons
    fprintf([repmat('\b',1,4) '%s\n'],num2str(sprintf('%03d',neuron)));
    
    figure('Position', [200 120 900 600],'Visible','off');
        subplot(2,10,2:5);
            ...
            %hold on;  
            plot(5*movmean(Neurons.SummedPlaceActivity(neuron).Dir1,Params.smoothActivitySpace),'LineWidth',2,'Color','m');
            yticklabels({})
            xlabel('Position (cm)')
            if Neurons.isPC(neuron).Dir1
               ...
               ... %'Peak at ' num2str(100*round(Neurons.Peak(neuron).Dir1,3)) '% of theor. max ; Stability : ' num2str(100*round(Neurons.Stab(neuron).Dir1,3)) '%']) 
                 'Act in PF = ' num2str(Neurons.ActvityInPFvsOutPF(neuron).Dir1, '%.1f') ' x act out PF; Stability: ' num2str(100*round(Neurons.Stab(neuron).Dir1,3)) '%'])
        ...
        subplot(2,10,7:10);
            ...
            %hold on; 
            plot(5*movmean(Neurons.SummedPlaceActivity(neuron).Dir2,Params.smoothActivitySpace),'LineWidth',2,'Color','m');
            yticklabels({})
            xlabel('Position (cm)')
            if Neurons.isPC(neuron).Dir2
                ...
                'Act in PF = ' num2str(Neurons.ActvityInPFvsOutPF(neuron).Dir2, '%.1f') ' x act out PF; Stability: ' num2str(100*round(Neurons.Stab(neuron).Dir2,3)) '%'])
             subplot(2,10,6)
