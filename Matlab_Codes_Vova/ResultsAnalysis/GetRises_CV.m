...
%_________ With small evts _________ 
    ...
    %     sAct=smoothdata(results.C_raw(:,1:SizeData),2,'sgolay',50);                                                   % Activity is smoothed calcium traces
        sAct=smoothdata(diff(smoothdata(results.C_raw(:,1:Experiment.SizeData),2,'sgolay',50),1,2),2,'sgolay',50);      % Activity is the smoothed derivative of smoothed calcium traces        
...
%                 figure; 
%                 ax1 = subplot(3,1,1);
%                 plot(results.C_raw(neuron, :)); hold on;   plot(results.C(neuron, :)); plot(FiltAct(neuron,:));
%                 ax2 = subplot(3,1,2);
