function [estimations,actual,errors,average] = ReconstructPositionRaly(positions,spikes,windows,varargin)
...
% USAGE
%
%    [estimations,positions,errors,average] = ReconstructPositionRaly(positions,spikes,windows,<options>)
%
...
%   NOTES FROM RALY TO FEDE
%       In the case of theta sequences, which happen on the maze, we may be interested in
%       a corrected probability matrix (i.e. the "true current position" is in the center,
%       and lower down are positions before and higher up are positions after the true position).
%       This corrected matrix is called "errors". To compute theta sequences, we group the error
%       matrices according to theta phase - this is the "average" output. All these other outputs
%       ("actual", "errors", "average") are not really applicable to replay analysis (maybe to awake
%       replay but probably not), so you can safely ignore them!
%
