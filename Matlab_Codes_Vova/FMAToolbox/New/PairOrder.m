...
% sequenceList can be a matrix with NaNs, e.g.
%
%       1   3   4   2   1   6   NaN
%       NaN 2   NaN 1   2   3   5
%       4   1   1   5   2   6   6
...
% EXAMPLE:
% GetFiringAngles(Restrict(positions,PAF), Restrict(spikes,PAF));
% thetaScores = PairOrder(,GetSequenceList(spikes, Restrict(theta,period1)));
...
occurrences = Accumulate([column(~isnan(sequenceList)) sequenceList(~isnan(sequenceList))]); %How many times each neuron fired in each sequence
