...
if isempty(psf)
    ...
    HY = imfilter_border(reshape(Y, d1,d2,[]), psf);
    mask = reshape(Y, d1,d2,[]);
    mask = mask(:,:,1:100);   
    mask = (max(mask,[],3)) >0;
    HY = HY .* repmat(mask, [1 1 size(HY, 3)]);
end
...
while searching_flag && K>0
    ...
    v_max = ordfilt2(v_search, tmp_d^2, true(tmp_d)); % prend le max sur des kernels de tmp_d x tmp_d pixels
    ...
    for mcell = 1:length(ind_localmax)
        ...
        y0 = HY(ind_p, :); % trace temporelle du seed pixel en cours d'examen
        y0_std = std(diff(y0)); % evaluation du bruit haute frequence
        %         y0(y0<median(y0)) = 0;
        %         if (k>=1) && any(corr(Cin(1:k, :)', y0')>0.9) %already found similar temporal traces
        %             continue;
        %         end
        if max((y0))< 3*y0_std % !!!!! je change ici !!!!!!! (par rapport � la ligne d'apres)
%       if max(diff(y0))< 3*y0_std % signal is weak
        ...
        %% extract ai, ci
        sz = [nr, nc];
        if options.center_psf % etonnante cette boucle if ou on fait la meme chose dans les deux cas
            [ai, ci_raw, ind_success] =  extract_ac(HY_box, Y_box, ind_ctr, sz, options.spatial_constraints, options.deconv_options.window_baseline);
        else
            [ai, ci_raw, ind_success] =  extract_ac(HY_box, Y_box, ind_ctr, sz, options.spatial_constraints, options.deconv_options.window_baseline);
        end
        ...
        if ind_success
            ...
            ind_search(ind_nhood(ai>max(ai)*0.7)) = true; % !!!! J'ai change le *0.5 en *0.7 pour que les neurones puisse �tre plus proches. Ca va prendre plus de temps...
