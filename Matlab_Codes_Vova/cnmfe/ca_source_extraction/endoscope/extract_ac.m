function [ai, ci, ind_success] = extract_ac(HY, Y, ind_ctr, sz, spatial_constraints, window_baseline)
...
nr = sz(1);
nc = sz(2);
min_corr = 0.9; % au lieu de 0.9 !!!!!! mais pourquoi donc redefinit-on ca et ne prend on pas des donnees definies dans les options ????? et pourquoi 0.9 et pas 0.7 comme dans le papier ?
min_pixels = 5; % au lieu de 5 !!!!!! mais pourquoi donc redefinit-on ca et ne prend on pas des donnees definies dans les options ?????
...
if sum(ai(:)>0) < min_pixels %the ROI is too small
    ind_success=false;
   % return;  % !!!!! Je supprime �a car �a fait planter le programme ! on
   % sort de la boucle et on a une erreur, c'est tout...
end
...
% CATHIE - remove the original method of subtracting the baseline
% we use two methods for estimating the noise level 
% [~, sn] = estimate_baseline_noise(ci); 
% psd_sn = GetSn(ci); 
% if sn>psd_sn
%     sn =psd_sn; 
...
% CATHIE - method to remove baseline on a sliding window. 
[ci, ~] = remove_baseline_sliding_window(ci, 10, window_baseline); %10th percentile
