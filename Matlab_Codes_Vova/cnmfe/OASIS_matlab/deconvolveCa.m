...
if isempty(options.pars) || all(options.pars==0)
    switch options.type
        ...
        case 'ar2'
            ...
            %tau_r = 4; % en nb de frames
            %tau_d = 14;
            tau_r = options.Tar2.tau_r ;
            tau_d = options.Tar2.tau_d ;
            tau_dr = [tau_d, tau_r];
            options.pars = exp2ar(tau_dr);
...
switch lower(options.method)
    ...
    case 'bsd'
        [Ninf,c,Palg,~,Oalg] = BSD(y, options.Obsd, options.Pbsd); % Performs deconvolution.
        
        % dans BSD Palg.th = options.Pbsd.th
        % si Palg.th = 0, on s'en tient la et on sort c. 
        % sinon on utilise Palg.th pour eliminer les spikes d'amplitude trop
        % faible
        ...
        elseif Palg.th < 0
            ...
            gamma = exp(-Oalg.dt/Palg.tauRise)+exp(-Oalg.dt/Palg.tauDecay)-exp(-Oalg.dt/Palg.tauRise - Oalg.dt/Palg.tauDecay) ;
            eta = (Palg.tauRise/Palg.tauDecay)^(Palg.tauDecay/(Palg.tauDecay - Palg.tauRise))*( Palg.tauDecay/Palg.tauRise - 1)./(exp(-Oalg.dt/Palg.tauDecay)-exp(-Oalg.dt/Palg.tauRise));
            UnderTresh = Ninf < (-Palg.th)* Pphys.threshold;
            s = Ninf; 
            s(UnderTresh) = 0; 
            c=filter(1,[eta, -eta * (gamma+delta)  ,  eta * delta  ],s);
