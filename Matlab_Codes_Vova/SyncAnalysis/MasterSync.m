...
%     median(errorCm(1:floor(end/2)))
 ...
 %   load('Px.mat'); load('Lambda.mat');                                                                     % Load Data from Train Phase                                                                      
    ...
    % test; On raccourcit la matrice des evts car ça prend beaucoup de
    % temps si je fais la détection des évnts sans la condition sur la
    % dérivée. 
...
%    Test the reconstruction on Awake itself (just to evaluate the performance)
   ...
   % train
    ...
    % Test on sleep
    EvtMat2 = EvtMat;%(:,1:20000);                                                                            % On raccourcit la matrice des evts car ça prend beaucoup de temps si je fais la détection des évnts sans la condition sur la dérivée. 
    spikes=[];                                                                                              % Initialize 'spikes' (starts) sample mat.
    for n=1:Experiment.nNeurons                                                                             % Loop on neurons
        spikeTimesN = find(EvtMat2(n,:)); 
        spikes=[spikes ;  spikeTimesN' n*ones(numel(spikeTimesN),1)];                     % Add neuron ID and starts idx to the spikes sample
...
%         
%            EvtMat3 = EvtMat(:,iSCE - nwinplot*(wndw/0.01)+2  : iSCE + nwinplot*(wndw/0.01)-1);                                                                            % On raccourcit la matrice des evts car ça prend beaucoup de temps si je fais la détection des évnts sans la condition sur la dérivée. 
